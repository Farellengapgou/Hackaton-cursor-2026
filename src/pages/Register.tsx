    ...
    <div className="flex min-h-screen bg-themed">
      ...
      <div className="flex flex-1 flex-col items-center justify-center p-8">
        ...
        <div className="w-full max-w-md">
          <h2 className="text-2xl font-bold text-themed-fg">{t('auth.registerTitle')}</h2>
          <p className="mt-1 text-sm text-muted">{t('auth.registerSubtitle')}</p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase text-muted">{t('auth.name')}</label>
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full rounded-lg border border-themed bg-themed-panel px-4 py-3 text-themed-fg outline-none focus:border-primary"
              />
            </div>
            <div>
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase text-muted">{t('auth.confirmPassword')}</label>
              <input
                type="password"
                value={confirm}
