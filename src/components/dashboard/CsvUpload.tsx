...
export default function CsvUpload() {
  ...
  const onDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (file?.name.endsWith('.csv')) await handleUpload(file);
    },
    [handleUpload],
  );
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none">
        ...
        <p className="mt-1 text-xs text-muted">{t('dashboard.uploadSubtitle')}</p>
      ...
      {uploadLoading && (
        <div className="absolute inset-x-0 bottom-0 h-1 overflow-hidden rounded-b-xl bg-themed-skeleton">
          <div className="h-full w-1/3 bg-primary" style={{ animation: 'slide 1.2s infinite' }} />
        </div>
      )}
    </div>
  );
